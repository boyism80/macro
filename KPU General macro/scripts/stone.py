...
def engraving(app, name):
    found = app.Detect(name, area={"x": 618, "y": 337, "width": 102, "height": 87}, untilDetect=False)
    if name in found:
        return 1

    found = app.Detect(name, area={"x": 621, "y": 428, "width": 99, "height": 92}, untilDetect=False)
    if name in found:
        return 2
